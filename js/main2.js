document.addEventListener('DOMContentLoaded', () => {
  ...
  const initPage = () => {
    ...
    const existingCustomer = JSON.parse(localStorage.getItem('DRFCustomerInfo'));
      ...
      // ✅ Modal login/signup toggle logic for #modallogin
      const modal = document.getElementById('modallogin');
      const loginBtn = document.getElementById('openLoginBtn'); // Ensure this button exists
      const switchToSignup = document.getElementById('switchToSignup'); // Add these buttons inside modal
      const switchToLogin = document.getElementById('switchToLogin');
      const closeModal = modal?.querySelector('.modal-close'); // e.g., a <span class="modal-close">&times;</span>
  ...
  window.onload = function () {
    ...
    google.accounts.id.renderButton(
      document.getElementById("googleSignInDiv"), // ID of your button container
  ...
  if (switchToSignUp && switchToLogin && loginForm && registerForm) {
  ...
  if (googleLoginBtn) {
    googleLoginBtn.addEventListener('click', function () {
      window.location.href = '/auth/google-login.php'; // Replace with actual path
    });
  }
